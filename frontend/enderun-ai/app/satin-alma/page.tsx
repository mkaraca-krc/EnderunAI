...
import { purchaseRequestService, PurchaseRequestListItem } from "@/services/purchase-request.service";

const statusLabels: Record<number,string> = {0:"Taslak",1:"Onaya Gönderildi",2:"Onaylandı",3:"Teklif",4:"Sipariş",5:"Tamamlandı",6:"İptal",7:"Reddedildi"};
const priorityLabels: Record<number,string> = {0:"Düşük",1:"Normal",2:"Yüksek",3:"Kritik"};
const today = new Date().toISOString().slice(0,10);

type Line = { materialDescription:string; quantity:string; unit:string; requestedDeliveryDate:string; notes:string };
const emptyLine = (): Line => ({ materialDescription:"", quantity:"", unit:"Adet", requestedDeliveryDate:"", notes:"" });
...
export default function PurchasingPage(){
  ...
  async function create(e:FormEvent){e.preventDefault();setSaving(true);setError("");setSuccess("");
    try{await purchaseRequestService.create({companyId:form.companyId,projectId:form.projectId,requestNumber:form.requestNumber,requestDate:form.requestDate,neededByDate:form.neededByDate||null,requestedByName:form.requestedByName,description:form.description||null,priority:Number(form.priority),items:form.items.map(x=>({materialDescription:x.materialDescription,quantity:Number(x.quantity),unit:x.unit,requestedDeliveryDate:x.requestedDeliveryDate||null,notes:x.notes||null}))});setSuccess("Satın alma talebi taslak olarak oluşturuldu.");setShowForm(false);setForm(v=>({...v,projectId:"",requestNumber:"",neededByDate:"",requestedByName:"",description:"",priority:"1",items:[emptyLine()]}));await load();}
