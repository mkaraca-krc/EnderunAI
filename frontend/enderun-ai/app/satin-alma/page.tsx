...
export default function PurchasingPage(){
  /**
   * Düğme -> uç -> izin:
   *   POST purchase-requests -> purchasing-requests.create
   *
   * ROTA KAPISI YETMİYOR: "/satin-alma" purchasing.view istiyor —
   * listeyi görmek ile talep AÇMAK ayrı yetkiler.
   */
  ...
  return <ErpShell design="redwood" title="Satın Alma Talepleri" description="Proje bazlı talep ve onay süreçleri">
    ...
    <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
      ...
      {actions.can("create")&&<Button onClick={()=>setShowForm(v=>!v)}>{showForm?"Formu Kapat":"+ Yeni Talep"}</Button>}
    </div>
    {showForm&&<Card className="mb-6"><CardHeader><h2 className="text-lg font-semibold text-slate-900">Yeni Satın Alma Talebi</h2></CardHeader><CardContent><form onSubmit={create}>
      ...
      <div className="mt-6 flex justify-end gap-3"><Button type="button" variant="secondary" onClick={()=>setShowForm(false)}>Vazgeç</Button>{actions.can("create")&&<Button type="submit" loading={saving}>Taslak Kaydet</Button>}</div>
