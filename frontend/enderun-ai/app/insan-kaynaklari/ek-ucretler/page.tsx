...
import { currencyMoney, date } from "@/lib/format/turkish";
...
export default function AdditionalCompensationPage() {
  ...
  /** Süzgeçlere uyan TOPLAM kayıt — uçtan geliyor, listeden sayılmıyor. */
   ...
   * için: Excel'e giden metin ile ekrandaki rozet aynı yerden türüyor.
   */
  const ekUcretSutunlari: DataTableColumn<CompensationComponent>[] = [
    {
      key: "personel",
      header: "Personel",
      value: (row) =>
        personnelById.get(row.personnelId)?.fullName ?? row.personnelId,
      render: (row) => {
        const person = personnelById.get(row.personnelId);

        return (
          <>
            <strong className="block">{person?.fullName ?? row.personnelId}</strong>
            <small className="text-slate-500">{person?.employeeNumber}</small>
          </>
        );
      },
    },
    {
      ...
      value: (row) => {
        ...
        return project ? `${project.code} - ${project.name}` : "Genel";
      },
    },
    {
      key: "kod",
      header: "Kod / Ad",
      value: (row) => `${row.code} — ${row.name}`,
      render: (row) => (
        <>
    ...
    },
    {
      key: "tur",
      header: "Tür",
      value: (row) => componentTypeLabel(row.componentType),
    },
    {
      key: "hesaplama",
      header: "Hesaplama",
      value: (row) => calculationTypeLabel(row.calculationType),
    },
    {
      key: "odeme",
      header: "Ödeme",
      value: (row) => paymentMethodLabel(row.paymentMethod),
    },
    {
      key: "tutar",
      header: "Tutar",
      // TUTAR SÜTUNU SAĞA HİZALI, sabit genişlikli rakamlarla:
      // basamaklar alt alta gelsin, gözle karşılaştırılabilsin.
      align: "right",
      value: (row) => money(Number(row.amount), row.currencyCode),
    },
    {
      key: "gecerlilik",
      header: "Geçerlilik",
      value: (row) =>
        `${date(row.effectiveStartDate)} — ${
          row.effectiveEndDate ? date(row.effectiveEndDate) : "Süresiz"
        }`,
      render: (row) => (
        <>
          <span className="block">{date(row.effectiveStartDate)}</span>
    ...
    },
    {
      ...
      value: (row) =>
        [
          row.isAttendanceBased ? "Puantaj" : null,
          row.includeInPayroll ? "Bordro" : null,
          row.includeInProjectCost ? "Proje" : null,
          row.includeInSgkBase ? "SGK" : null,
          row.includeInIncomeTaxBase ? "Gelir V." : null,
        ]
          .filter(Boolean)
          .join(", "),
      render: (row) => (
        <div className="flex max-w-72 flex-wrap gap-1">
          {row.isAttendanceBased && (
    ...
    },
    {
      key: "durum",
      header: "Durum",
      value: (row) => (row.isActive ? "Aktif" : "Pasif"),
      render: (row) => (
        <span
  ...
  async function loadData() {
    ...
    try {
      ...
      // Uç sayfalı yanıt döndürüyor; toplam da geliyor ve
      // "Toplam X kayıt" bilgisi artık listeden sayılmıyor.
  ...
  /*
   * SAYFA VEYA BOYUT DEĞİŞİNCE YENİDEN YÜKLE.
   *
   * Sunucu kipinde tablo yalnız "hangi sayfa istendi" bilgisini
   * bildiriyor; veriyi ekran çekiyor. Bu effect olmazsa sayfalayıcı
   * tıklanır, sayı değişir ama LİSTE AYNI KALIRDI.
   */
  useEffect(() => {
    void loadData();
    // `loadData` her renderda yeniden kuruluyor; bağımlılığa konsaydı
    // effect her renderda çalışır ve sonsuz döngü doğardı.
  ...
  useEffect(() => {
    async function loadPage() {
      try {
        ...
        setItems(componentResult.items);
        setTotal(componentResult.total);
  ...
  return (
    ...
    >
          ...
          <button
            ...
            onClick={() => {
              /*
                ...
                Dönmezse kullanıcı 7. sayfadayken filtreyi daraltınca
                sonuç 2 sayfaya inse bile 7. sayfa istenir ve BOŞ ekran
                gelir; kullanıcı "kayıt yok" sanır.

                Sayfa zaten 1'se effect tetiklenmez, o yüzden yükleme
                elle çağrılıyor.
              */
      ...
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          <DataTable
            rows={items}
            columns={ekUcretSutunlari}
            rowKey={(row) => row.id}
            title="Ek Ücretler"
            /*
              SUNUCU KİPİ: sayfa, boyut ve TOPLAM uçtan geliyor.
              Uç sayfalı yanıt döndürdüğü hâlde ekran ham tabloyu
              çizmeye devam etseydi kullanıcı yalnız ilk sayfayı
              görür, sayfalayıcı hiç çıkmazdı — sessiz kırpma.
            */
            server={{
              total,
              page,
            ...
            loading={loading}
            emptyText="Ücret bileşeni bulunamadı."
          />
      </section>
      {pending && (
        <ConfirmDialog
