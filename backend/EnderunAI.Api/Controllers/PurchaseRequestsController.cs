...
public sealed class PurchaseRequestsController(AppDbContext db) : ControllerBase
{
        ...
        CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(request.RequestNumber))
            return BadRequest("Talep numarası zorunludur.");
        if (request.Items.Count == 0)
            return BadRequest("En az bir talep kalemi eklenmelidir.");
        if (request.Items.Any(x => x.Quantity <= 0))
            return BadRequest("Talep miktarları sıfırdan büyük olmalıdır.");

        var duplicate = await db.PurchaseRequests.AnyAsync(
            x => x.CompanyId == request.CompanyId && x.RequestNumber == request.RequestNumber,
            cancellationToken);
        if (duplicate)
            return Conflict("Bu talep numarası daha önce kullanılmış.");

        var materialIds = request.Items.Select(x => x.MaterialId).Distinct().ToList();
        var existingMaterialCount = await db.Materials.CountAsync(
            x => materialIds.Contains(x.Id), cancellationToken);
        if (existingMaterialCount != materialIds.Count)
            return BadRequest("Talep kalemlerinden en az biri geçerli bir malzeme kartına bağlı değil.");
    ...
    public async Task<ActionResult> Submit(Guid id, CancellationToken cancellationToken)
    {
        ...
        if (entity.Status != PurchaseRequestStatus.Draft)
            return Conflict("Yalnızca taslak talepler onaya gönderilebilir.");
        if (entity.Items.Count == 0)
            return Conflict("Kalemsiz talep onaya gönderilemez.");
    ...
    public async Task<ActionResult> Approve(Guid id, CancellationToken cancellationToken)
    {
        ...
        if (entity.Status != PurchaseRequestStatus.PendingApproval)
            return Conflict("Yalnızca onay bekleyen talepler onaylanabilir.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (entity is null)
            return NotFound();
        if (entity.Status != PurchaseRequestStatus.PendingApproval)
            return Conflict("Yalnızca onay bekleyen talepler reddedilebilir.");
        if (string.IsNullOrWhiteSpace(request.Reason))
            return BadRequest("Red nedeni zorunludur.");
