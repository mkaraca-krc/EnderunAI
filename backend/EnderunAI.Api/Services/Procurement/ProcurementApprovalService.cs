...
{
    public async Task<ProcurementApprovalInstance> SubmitPurchaseOrderAsync(
        Guid orderId,
        ApprovalActor actor,
        CancellationToken cancellationToken = default)
    {
        ...
        var instance = new ProcurementApprovalInstance
        {
            ...
            Steps = orderedSteps.Select(x => new ProcurementApprovalInstanceStep
            {
                ...
                Status = rule.FlowMode == ApprovalFlowMode.Parallel || x.SequenceNo == firstSequence
        ...
        instance.History.Add(new ProcurementApprovalHistory
        {
            ...
            Comment = "Satın alma siparişi onaya gönderildi."
        });
    ...
    public async Task<ProcurementApprovalInstance> ActAsync(
        Guid instanceId,
        Guid stepId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var order = await appDb.PurchaseOrders.FirstOrDefaultAsync(
            x => x.Id == instance.DocumentId,
            cancellationToken) ?? throw new InvalidOperationException("Bağlı satın alma siparişi bulunamadı.");
