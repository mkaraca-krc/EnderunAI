...
/// <summary>
/// Satın alma dashboard'unun tek kaynağı.
///
/// NEDEN SERVİS: bu hesap eskiden controller'ın içindeydi ve yönetim
/// KPI'ları aynı sayıyı ikinci kez hesaplamak zorunda kalacaktı. İki
/// ayrı sorgu zamanla ayrışır ve aynı ekranda iki farklı "açık
/// sipariş" sayısı çıkardı. Şimdi hem uç hem KPI buradan okuyor.
///
/// Kod controller'dan OLDUĞU GİBİ taşındı; davranış değişmedi.
/// Tek fark: veri kapsamı (scope) dışarıdan parametre olarak geliyor,
/// böylece servis HTTP bağlamına bağımlı değil.
/// </summary>
public sealed class ProcurementDashboardService(AppDbContext db)
{
    public async Task<ProcurementDashboardResponse> GetAsync(
        Guid? companyId,
        Guid? projectId,
